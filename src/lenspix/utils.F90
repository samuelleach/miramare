...
!Set up start indices and get total number of steps
 ...
 function concat_s_n(S1,N2,S3,N4,S5,N6,S7,N8,S9,N10,S11) result(concat)
   character(LEN=*), intent(in) :: S1
   integer, intent(in) :: N2
   character(LEN=*), intent(in) , optional :: S3, S5, S7, S9,S11
...
!  end function IntToStr
  ...
  ! From comm_utils.f90 by Hans Kristian Eriksen
  ...
  function DoubleToStr(R, figs)
    ...
    if (abs(R)>=0.001) then
       write (DoubleToStr,'(f12.6)') R
       
       DoubleToStr = adjustl(DoubleToStr)
       if (present(figs)) then
          DoubleToStr = DoubleToStr(1:figs)
       else
          DoubleToStr = DoubleToStr(1:6)  
       end if
       
    else
       if (present(figs)) then
          write (DoubleToStr,'(F'//trim(numcat('(',figs))//'.2)') R
       else
          write (DoubleToStr,'(G9.2)') R
       end if
       DoubleToStr = adjustl(DoubleToStr)
    end if
...
!   character(LEN=8192) :: InLine
!   character(LEN=16384) :: InLine
   character(LEN=32768) :: InLine
...
!Subroutine by Alan Miller. Added to utils.F90 by SL.
  ...
  double precision function GAUSSIAN1()
    ...
    if (use_ziggurat) then
     ...
     if (ISET==0) then
        ...
        GSET=V1*FAC
        GAUSSIAN1=V2*FAC
!        ISET=1 !bug?   SML . This line seems to give periodicity 2 to the initrandom() subroutine.
      else
        GAUSSIAN1=GSET
        ISET=0
      endif
