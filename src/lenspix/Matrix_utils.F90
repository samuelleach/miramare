...
!Query
 ...
 function MatrixSym_LogDet_v2(mat) result (logDet)
  real(dm), intent(in) :: mat(:,:)
  real(dm) logDet
  real(dm) Tmp(size(mat,dim=1),size(mat,dim=1))
  integer i
  ...
  logDet = 1.d0
  do i=1, size(mat,dim=1)
   logDet = logDet *tmp(i,i)*tmp(i,i)
  end do
  logDet = log(logDet)
      
 end function MatrixSym_LogDet_v2

  
   subroutine Matrix_CRotateSymm(Mat,U,m,Out)
    !Gets U^dag Mat U
     integer, intent(in) ::m
   ...
   subroutine Matrix_RotateSymm(Mat,U,m,Out)
    !Gets U^T Mat U
     integer, intent(in) ::m
  ...
  function Matrix_is_positive_definite(M)
    !Added by SL
    real(dm), intent(inout):: M(:,:)
    logical Matrix_is_positive_definite
    integer n, info
...
!     call Matrix_End('Inverse')
  ...
  subroutine Matrix_Inverse_new(M)
   !Inverse of symmetric positive definite matrix
     real(dm), intent(inout):: M(:,:)
     integer i, n
...
!    write (*,*), 'min/max eigenvalues = ', minval(w), maxval(w)
    if (any(w .eq. 0)) then
         write (*,*) 'min/max eigenvalues = ', minval(w), maxval(w)
         call MpiStop('Matrix_Inverse: zero eigenvalues')  
    end if
!    if (any(w<=0)) then
!         write (*,*), 'min/max eigenvalues = ', minval(w), maxval(w)
!         call MpiStop('Matrix_Inverse: negative or zero eigenvalues')  
!    end if
  ...
  end subroutine Matrix_Inverse_new
