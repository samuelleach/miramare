...
contains
 ...
 subroutine conjgrad_wrapper(Params,ftol_in,exitstatus)
  ...
  ftol = ftol_in
  print *, 'convervence criterion: delta_loglike = ',ftol_in
...
!  tol= 0.1   ! get step length good to 10 per cent accuracy
  scale = 0.8 ! change parameters by about 20 percent of their estimated widths. A guess
  tol   = 0.1   ! get step length good to 10 per cent accuracy
...
! scale the params so they are all roughly the same order of magnitude
  ...
  Bestfit_loglike= fun
  if (Feedback>0) write(*,*) 'Found minimum value of -Log(Like) = ',fun
  if (exitstatus.eq.0) then
    write (*,*) 'Incredible! Loglike changed by less than ',eps,' so must have converged'
        Params=CurParams
  elseif (exitstatus.eq.1) then
    if (Feedback>0) write(*,*) 'Converged sucessfully (loglike changes by less than',ftol,')'
    if (Feedback>0) write(*,*) ' after ',its,' iterations'
        Params=CurParams
...
!       write(*,*) 'Exiting because we seem to be going up hill now'
 ...
 subroutine OutOfBoundsParameter(Params)
   ...
   do i=1, num_params_used
      ii=params_used(i)
      write(*,*) 'Parameter ',ii, '=', Params%P(ii)
   end do
   call DoStop
 end  subroutine OutOfBoundsParameter
...
! scale the params so they are all roughly the same order of magnitude
  ...
  ffn = GetLogLike(P)
  if (ffn.eq.LogZero) call OutOfBoundsParameter(P)
...
!        if (Feedback>1) write(*,*) 'gradient wrt ',i,'th element is: ',df(i)
 ...
 subroutine step1d(i,vect,delta,OtherParams,funcvect,dfplus,dfminus)
  ...
  real  dfplus,dfminus
    ...
    if (funcvectplus.eq.LogZero) call OutOfBoundsParameter(OtherParams)
    ...
    if (funcvectminus.eq.LogZero) call OutOfBoundsParameter(OtherParams)
...
!c        if(2.*abs(fun-funprev).le.ftol*(abs(fun)+abs(funprev)+eps)) then
        if(abs(fun-funprev).le.ftol) then
          if (Feedback>0) write(*,'(a,f10.8,a,i6,a)')   &
...
!                  ,lambda1,', where gradient is ',df1
        if (v>1) write(*,*) 'End of iteration ',its
        its=its+1
