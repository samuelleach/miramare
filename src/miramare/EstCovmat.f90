...
contains
 ...
 function EstCovmat(Params, bestdll_in, sucess)
   integer :: ntries = 200 ! Maximum number of attempts to get a covmat with all +ve evals
!   integer :: ntries = 100 ! Maximum number of attempts to get a covmat with all +ve evals
...
!      call GetHessdiagonal(CenterParams,parsteps,Hess)
   ...
   ! uncomment this bit to update the propose_matrix..
    if (sucess>0) then
      U=Hess
      call Matrix_Inverse1(U) ! gives the covariance matrix
...
!                write(4,*) CenterParams%P(ii),CenterParams%P(jj),wii,wjj,Lgrid
 ...
 subroutine GetHessDiagonal(CenterParams,parsteps,Hess)
   ! Get the curvature matrix (Hess) diagonal elements for the grid defined by CenterParams
   ! and parsteps. 
   ! Return the answer in Hess.
   ! Input only: CenterParams, parsteps
   ! Ouput only: Hess
...
!       end do
   ...
   do i=1, num_params_used
      print *,i,hess(i,i)
!      if (hess(i,i) .lt. 0.) hess(i,i)= - hess(i,i)
   end do
               
 end subroutine GetHessDiagonal
